...
void solve()
    {
        ...
        ans.pb(s[0]);
        for(int i=1;i<=n-4;){
            if(s[i]=='d' and s[i+1]=='o' and s[i+2]=='t')ans.pb('.'),vis[i]=true,vis[i+1]=true,vis[i+2]=true,i+=3;
            else {
                    if(!fir and s[i]=='a' and s[i+1]=='t')ans.pb('@'),vis[i]=true,vis[i+1]=true,fir=true,i+=2;
                    else ans.pb(s[i]),vis[i]=true,i++;
            }
        }
        if(!fir){
            if(n-3 >0 )
            {
                if(s[n-3]=='a' and s[n-2]=='t')ans.pb('@'),vis[n-3]=true,vis[n-2]=true;
            }
        }
        rep(i,0,n,1){
            if(!vis[i])ans.pb(s[i]);
