...
bool op(const pll &a,const pll &b){return a.yt>b.yt;}
...
void solve()
    {
        ...
        vector<pll>arr(m);
        rep(i,0,m,1){
            cin>>arr[i].xt>>arr[i].yt;
        }
       ll sum=0;
       rep(i,0,n,1)sum+=val[i],val[i]=sum;
       ll ans=sum;
     //  cout<<ans<<endl;
       rep(i,0,m,1){
           ll cnt=lower_bound(all(val1),arr[i].yt)-val1.begin();
         if(cnt>0)
         {  if(cnt<=arr[i].xt){
               ans=getmax(ans,sum-val[cnt-1]+cnt*arr[i].yt);
           }
           else {
               ans=getmax(ans,sum-val[arr[i].xt-1]+arr[i].xt*arr[i].yt);   
            }
           }
          // cout<<ans<<endl;
       }
