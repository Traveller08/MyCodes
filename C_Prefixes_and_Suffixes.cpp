...
void solve()
    {
        ...
        ll m=2*n-2;
        rep(i,0,m,1){
            vector<pair<string,ll>>pre,suff;
            pre.pb(val[i]);
            rep(j,i+1,m,1){
                if(i!=j){
                    string temp=pre.back().xt;
                    ll mn=getmin(temp.size(),val[j].xt.size());
                    bool found=0;
                    rep(k,0,mn,1){
                       if(val[j].xt[k]!=temp[k]){
                           found=true;
                           break;
                       }
                    }
                    if(!found)pre.pb(val[j]);
                    else suff.pb(val[j]);
                }
            }
           // for(auto c:pre)cout<<c.xt<<endl;cout<<endl;
            bool ans=0;
            ll mn=suff.size();
            rep(j,1,mn,1){
                string tp1=suff[j].xt;
                string tp2=suff[j-1].xt;
           //     cout<<tp1<<" "<<tp2<<endl;
                ll pp=getmin(tp1.length(),tp2.length());
                ll it1=tp1.length()-1;
                bool pf=0;
                ll it2=tp2.length()-1;
                while(pp--){
          ...
          //  for(auto c:suff)cout<<c.xt<<endl;cout<<endl;
        //    cout<<boolalpha<<ans<<endl;
            if(!ans){
                ...
                for(auto c:fin)cout<<c;cout<<endl;
                break;
            }
        }
    }
