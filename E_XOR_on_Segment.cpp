...
vector<long long int> st(400009);
...
void update_tree(ll s,ll e,ll curr,ll l,ll r,vl &val,ll dx)
{
    ...
    ll mid=s+(e-s)/2;  
     update_tree(s,mid,2*curr+1,l,r,val,dx);
   update_tree(mid+1,e,2*curr+2,l,r,val,dx);
   st[curr]=st[2*curr+1]+st[2*curr+2];
}
void solve()
   {
       ...
       w(m){
           ...
           if(pq==1){
             ...
             cout<<query_ans(0,n-1,0,l-1,r-1)<<endl;
               ...
               ni3(l,r,dx);
               update_tree(0,n-1,0,l-1,r-1,val,dx);
           }
       }
   }
