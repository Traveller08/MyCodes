...
bool op(eventDetails a,eventDetails b){
...
void solve()
    {
        ni1(n);
        struct eventDetails val[n];
        rep(i,0,n,1){
            cin>>val[i].from>>val[i].to;
            val[i].index=i;
        }
        sort(val,val+n,op);
        vl ans(n,0);
        ...
        rep(i,0,n,1){
            for(ll j=val[i].from;j<=val[i].to;j++){
                if(!vis[j]){
                    vis[j]=true;
                  ans[val[i].index]=j;
           //       cout<<val[i].index<<" "<<j<<endl;
                  break;
                }
            }
        ...
        // rep(i,0,n,1){
        //    cout<<val[i].from<<" "<<val[i].to<<" "<<val[i].index<<endl;
        // }
