...
int find_set(int v,vl &parent) {
    if (v == parent[v])
        return v;
    return parent[v] = find_set(parent[v],parent);
}
void make_set(int v,vl &parent,vl &ranks) {
    parent[v] = v;
    ranks[v] = 0;
}

void union_sets(int a, int b,vl &parent,vl &ranks) {
    a = find_set(a,parent);
    b = find_set(b,parent);
    if (a != b) {
        if (ranks[a] < ranks[b])
...
void solve()
    {
        ni3(n,m1,m2);
        vl parent(n+1,-1),ranks(n+1),parent1(n+1,-1),ranks1(n+1);
         set<pll>ans,ans1,final_ans;
        rep(i,0,n+1,1){
            make_set(i,parent,ranks);
            make_set(i,parent1,ranks1);
        }
        rep(i,0,m1,1){
            ni2(p,q);
            union_sets(p,q,parent,ranks);
           
        }
         rep(i,0,m2,1){
            ni2(p,q);
            union_sets(p,q,parent1,ranks1);
            
        }
        rep(i,1,n+1,1){
            rep(j,i+1,n+1,1){
                 ll a=find_set(i,parent);
                    ll b=find_set(j,parent);
                    ll c=find_set(i,parent1);
                    ll d=find_set(j,parent1);
                    if(a!=b and c!=d){
                         ans.insert(mp(i,j));
                         union_sets(i,j,parent,ranks);
                         union_sets(i,j,parent1,ranks1);
                         if(ans.size()==n)break;
