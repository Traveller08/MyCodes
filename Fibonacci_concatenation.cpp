...
inline ll mod_mul(ll a, ll b, ll m)
{
    a = a % m;
    b = b % m;
    return (((a * b) % m) + m) % m;
}
inline ll mod_sub(ll a, ll b, ll m)
{
    a = a % m;
    b = b % m;
    return (((a - b) % m) + m) % m;
...
void precal()
{
    ...
    rep(i, 3, 100002, 1)
    {
       sz[i]=mod_add(ones[i - 1],sz[i - 1],mod)%mod;
        ones[i] = (sz[i - 1]);
    }
}
...
void solve()
{
    ...
    {
        ll one = ones[n]%mod;
        ll ss=mod_sub(sz[n],1,mod);
        ll occur = binPow(2,ss, mod);
        ll ans = mod_mul(one,occur,mod);
        cout << (ans)%mod << endl;
    }
}
