...
void build_tree(ll s,ll e,ll curr,vl &val)
{
   ...
   st1[curr]=st1[2*curr+1]+st1[2*curr+2];
}
long long int query_ans1(ll s,ll e,ll curr,ll l,ll r)
{
    if(l<=s and r>=e)return st[curr];
    if(l>e or r<s)return 0;
     ll mid=s+(e-s)/2;
     return query_ans1(s,mid,2*curr+1,l,r)+query_ans1(mid+1,e,2*curr+2,l,r);
}
long long int query_ans2(ll s,ll e,ll curr,ll l,ll r)
{
    if(l<=s and r>=e)return st1[curr];
    if(l>e or r<s)return 0;
     ll mid=s+(e-s)/2;
     return query_ans2(s,mid,2*curr+1,l,r)+query_ans2(mid+1,e,2*curr+2,l,r);
} 
void solve()
   {
       ni2(n,q);
       ...
       w(q){
          ni2(l,r);
          if((l-1)%2==0)cout<<query_ans1(0,n-1,0,l-1,r-1)<<endl;   
          else cout<<query_ans2(0,n-1,0,l-1,r-1)<<endl;  
       }
   }
