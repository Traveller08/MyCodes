...
#define graph_size 200005
vl adj[graph_size];
bool vis[graph_size];
ll ans=0;
vl val(graph_size),par(graph_size);
void dfs(ll v,ll mn,ll mx)
{
        vis[v]=true;
        mn=getmin(mn,val[v-1]);
        mx=getmax(mx,val[v-1]);
        ans=getmax(ans,mx-mn);
    for(auto x:adj[v]){
        if(!vis[x]){
            dfs(x,mn,mx);
            }
